...
export function ItalyComp() {
    ...
    const selectedClubData = italy?.leagueName?.find((league: League) => league.club === selectedClub);
        ...
        return (
            <div className="bg-slate-800 p-8 rounded-lg shadow-lg">
                ...
                <div className="bg-red-900/20 border border-red-500 p-6 rounded">
                    ...
                    <p className="text-slate-400 mt-2">Available clubs: {italy?.leagueName?.map((l: any) => l.club).join(", ")}</p>
                </div>
            </div>
        );
    }
